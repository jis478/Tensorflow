...
class UGATIT(object):
    ...
    def train(self):
        ...
        for iter in range(1, self.iterations + 1):
            ...
            if iter % self.sample_freq == 0:  # Only A->B
                ...
                fake_A2B, _, _ = self.genA2B(real_A)
                image_save(real_A, fake_A2B, self.image_path, iter, train=True)
    ...
    def test(self):
        ...
        # model initialization (Only A -> B)
        self.model = ResnetGenerator(output_nc=3, ngf=self.ch, n_blocks=self.n_res, light=self.light)
        
        # load the latest checkpoint from the given ckpt folder
        ckpt = tf.train.Checkpoint(model=self.model)
        manager = tf.train.CheckpointManager(ckpt, args.ckpt_dir)
        ...
        idx = 0
        for real_A in test_A:
            fake_A2B, _, _ = self.model(real_A)
            image_save(real_A, fake_A2B, self.image_path, idx, test=True)
            idx +=1 
