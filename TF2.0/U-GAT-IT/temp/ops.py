...
class Conv(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size, strides, pad, normal=None, act=None, use_bias=False, pad_type='REFLECT', **kwargs):
        ...
        if self.normal == 'IN':
            ...
            self.normal = SpectralNormalization
...
class ResnetAdaILNBlock(tf.keras.layers.Layer):
    ...
    def call(self, inputs, gamma, beta, smooting):
        x = tf.pad(inputs, [[0, 0], [1, 1], [1, 1], [0, 0]], mode='REFLECT')
        x = self.conv1(x)
        x = self.norm1(x, gamma, beta, smoothing)
        x = self.relu1(x)
        x = tf.pad(x, [[0, 0], [1, 1], [1, 1], [0, 0]], mode='REFLECT')
        x = self.conv2(x)
        x = self.norm2(x, gamma, beta, smoothing)
        x += inputs
        return x
...
class adaILN(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):  # 반드시 input_shape를 써야하는지? input shape에 상관 없으므로, 그냥 __init__에 넣어도 될 듯하다.
        self.rho = tf.Variable(initial_value=tf.fill([1, 1, 1, self.num_features], 1.0), dtype=tf.float32,
                               trainable=True, constraint=lambda x: tf.clip_by_value(x, clip_value_min=0.0, clip_value_max=1.0))
...
#         self.rho.assign(tf.clip_by_value(self.rho, clip_value_min=0.0, clip_value_max=1.0))
        return out
